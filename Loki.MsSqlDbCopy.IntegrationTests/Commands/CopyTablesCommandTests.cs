...
public class CopyTablesCommandTests : BaseMsSqlDbCopierIntegrationTests
{
    ...
    public async Task Execute_CopiesAllDatabaseTables_WhenCopyTablesIsSetToTrue()
    {
        ...
        var sourceConnectionStringBuilder =
            new SqlConnectionStringBuilder(SourceNorthWindDbContainer.GetConnectionString())
            {
                InitialCatalog = "Northwind"
            };
        ;

        var destinationConnectionStringBuilder =
            new SqlConnectionStringBuilder(DestinationNorthWindDbContainer.GetConnectionString())
            {
